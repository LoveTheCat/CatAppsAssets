...
- PNG favicons must pass: PNG signature, IHDR parse, complete chunk-boundary walk, CRC validation for every chunk, terminal IEND with no truncation, successful IDAT zlib/DEFLATE decompression, exact scanline byte-count validation for the declared dimensions/color type, and a complete pixel decode/reload with an independent image decoder. Container-level CRC validity alone is never sufficient. A malformed, truncated, or only partially decodable PNG is rejected even if some viewers can display it.
